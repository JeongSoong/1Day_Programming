...
{
    double angle_degree = DEG2RAD(m_angle_degree);
    Rotation_matrix[0][0] = cos(angle_radian); //x
    Rotation_matrix[0][1] = -sin(angle_radian); //x
    Rotation_matrix[1][0] = sin(angle_radian); //y
    Rotation_matrix[1][1] = cos(angle_radian); //y
}

void set_rotation_matrix_inverse(double m_angle_degree)
{
    double angle_degree = DEG2RAD(m_angle_degree);
    Rotation_matrix_inverse[0][0] = cos(angle_radian); //x
    Rotation_matrix_inverse[0][1] = sin(angle_radian); //x
    Rotation_matrix_inverse[1][0] = -sin(angle_radian); //y
    Rotation_matrix_inverse[1][1] = cos(angle_radian); //y
}
