...
void read_sonar_sensor(void){ //초음파센서 측정
  ...
  void nomal_average (int X){
    F_new_average=(F_old_average*(X-1)/X)+(F_Sonar_distance/X);
    F_old_average=F_new_average ;
    R_new_average=(R_old_average*(X-1)/X)+(R_Sonar_distance/X);
    R_old_average=R_new_average ;
    L_new_average=(L_old_average*(X-1)/X)+(L_Sonar_distance/X);
    L_old_average=L_new_average ;
  }
